...
(function () {
  ...
  function getGoogleTextProbe(resultBox = getResultBox()) {
    ...
    return (
      qs('.usGWQd .ryNqvb', resultBox) ||
      qs('.usGWQd .W297wb', resultBox) ||
      qs('.usGWQd .eDXd3b', resultBox) ||
      qs('.usGWQd [lang]', resultBox)
    );
  }
  ...
  function syncAiLayout(box = document.getElementById(AI_BOX_ID)) {
    ...
    const head = qs('.tm-gemma-head', box);
    const bodyWrap = qs('.tm-gemma-body-wrap', box);
    const toolbar = qs('.tm-gemma-toolbar', box);
    const body = qs('.tm-gemma-body', box);
    const probe = getGoogleTextProbe(resultBox);

    if (!head || !bodyWrap || !toolbar || !body || !probe) return;

    const resultRect = resultBox.getBoundingClientRect();
    const probeRect = probe.getBoundingClientRect();

    if (resultRect.width > 0 && probeRect.width > 0) {
      const padLeft = Math.max(24, Math.round(probeRect.left - resultRect.left));
      const padRight = Math.max(24, Math.round(resultRect.right - probeRect.right));

      bodyWrap.style.paddingLeft = `${padLeft}px`;
      bodyWrap.style.paddingRight = `${padRight}px`;
      toolbar.style.paddingLeft = `${padLeft}px`;
      toolbar.style.paddingRight = `${padRight}px`;
      head.style.paddingRight = `${padRight}px`;
    }

    const cs = getComputedStyle(probe);
    body.style.fontFamily = cs.fontFamily || 'Roboto, Arial, sans-serif';
    body.style.fontSize = cs.fontSize || '16px';
    body.style.fontWeight = cs.fontWeight || '400';
    body.style.lineHeight = cs.lineHeight || '1.45';
    body.style.letterSpacing = cs.letterSpacing || '0px';
  }
